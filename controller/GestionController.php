...
function actualizar_registro_archivo_gestion($id_archivo,$analizados,$insertados,$desactivados,$rechazados){
    ...
    $query = "UPDATE archivos_gestion SET fecha_procesamiento = '".$fecha_actual."', cantidad_analizados = ".$analizados.", cantidad_insertados = ".$insertados.", cantidad_desactivados = ".$desactivados.", cantidad_rechazados = ".$rechazados." WHERE id_archivo_gestion = ".$id_archivo."";
    //echo "query = ".$query."<br>";
    $resultado = $gestion->actualizar($query);
...
function analizar_contenido_archivo_gestion($ruta_archivo,$id_archivo,$tipo_insercion){
    ...
    for ($i = 0; $i < $sheetCount; $i++){
        ...
        if($cant_insertados > 0){
            $mensaje .= " Han sido insertados ".$cant_insertados." registros de nuevas lineas. Los numeros de linea insertados estan relacionados en el correo de notificacion.";
            $mensaje_notificacion .= " Han sido insertados ".$cant_insertados." registros de nuevas lineas. Los numeros de las lineas son los siguientes: ";
            $mensaje_notificacion .= ordernar_lineas($insertados);
        }

        if($cant_desactivados > 0){
            $mensaje .= " Han sido desactivadas ".$cant_desactivados." registros de lineas existentes. Los numeros de linea desactivados estan relacionados en el correo de notificacion.";
            $mensaje_notificacion .= " Han sido desactivadas ".$cant_desactivados." registros de lineas existentes. Los numeros de las lineas son los siguientes: ";
            $mensaje_notificacion .= ordernar_lineas($desactivados);
        }

        if($cant_rechazados){
            $mensaje .= " Han sido rechazados ".$cant_rechazados." registros para su gestion. Por favor asegurese que estos datos en el archivo sean consistentes para importarlos de nuevo. Los numeros de linea rechazados estan en el correo de notificacion";
            ...
            $mensaje_notificacion .= ordernar_lineas($rechazados);
        }

    }

    if(($cant_insertados > 0)||($cant_desactivados > 0)){
        //ACTUALIZAR EL REGISTRO EN LA BD CON LAS ESTADISTICAS OBTENIDAS DURANTE EL ANALISIS DEL ARCHIVO
        $num_reg = $num_reg - 1;
        actualizar_registro_archivo_gestion($id_archivo,$num_reg,$cant_insertados,$cant_desactivados,$cant_rechazados);
    }else{
