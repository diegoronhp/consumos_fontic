...
$destino_archivo = "";
$mensaje_email = "";
$origen_nuevos = $dir_raiz.'importados_interno/';
$origen_rectif = $dir_raiz.'importados_interno_rectif/';
...
if(($total_nuevos > 0)&&($total_rectif == 0)){
    ...
    $destino_archivo = $dir_raiz.'procesados_interno/';
...
if(($total_nuevos == 0)&&($total_rectif > 0)){
    ...
    $destino_archivo = $dir_raiz.'procesados_interno_rectif/';
    //echo "destino_archivo = ".$destino_archivo."<br>";
}
...
while($cant_archivos < $limite_archivos){
    ...
    if($rectificando){
        ...
        if($existe){
            ...
            $mensaje_email .= eliminar_registros_por_rectificar_archivo_interno($id_ini_datos,$id_fin_datos,$id_ini_voz,$id_fin_voz,$id_archivo);
            $mensaje_email .= insertar_registros_consumos_archivo_interno($origen_rectif,$nombre_archivo,$tipo_insercion);
            $mensaje_email .= "</p>";
        ...
        $tipo_insercion = 0;
        $mensaje_email = "<p>El archivo (".$nombre_archivo.") ha sido procesado y los resultados obtenidos son los siguientes: ";
        $mensaje_email .= insertar_registros_consumos_archivo_interno($origen_nuevos,$nombre_archivo,$tipo_insercion);
        $mensaje_email .= "</p>";
        enviar_correo_usuario($mensaje_email);
